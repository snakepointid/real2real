...
from real2real.models.seq2seq import NmtModel
from real2real.app.params import appParams
from real2real.preprocess.nmt_feeds import *
from real2real.utils.info_layout import *

def training():
        gpu_options = tf.GPUOptions(allow_growth = True)

        model = NmtModel(is_training=True)
        ...
        with tf.Session(graph = model.graph,config = tf.ConfigProto(gpu_options = gpu_options, allow_soft_placement = True, log_device_placement = False)) as sess:
                ...
                for epoch in range(appParams.num_epochs):
                        for source_batch,target_batch in cache['training']:
...
def evaluation():
        ...
        gpu_options = tf.GPUOptions(allow_growth = True)
        model = NmtModel(is_training=False)
        with tf.Session(graph = model.graph,config = tf.ConfigProto(gpu_options = gpu_options, allow_soft_placement = True, log_device_placement = False)) as sess:
                model.global_saver.restore(sess,FLAGS.restore_path+"/global_model")
                cache = LoadEvalFeeds()
                acc = [] 
                for raw_batch,source_batch,target_batch in cache:
                        ...
                        for idx,raw in enumerate(raw_batch):
                                if testa_pred[idx]!=target_batch[idx]:
                                        print("%s\t%s\tacc:%s\t%s\t%s"%(raw,probs[idx][target_batch[idx]]\
