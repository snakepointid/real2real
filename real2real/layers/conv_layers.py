...
def direct_conv(inputs,scope_name,is_training):
        static_shape  = inputs.get_shape()
        with tf.variable_scope(scope_name,reuse=None):
                filter_kernels = tf.get_variable('kernel', shape=[convLayerParams.kernel_size,static_shape[2],static_shape[2]],trainable=is_training)
                cnn_bias       = tf.get_variable('bias'  , shape=(static_shape[2],), initializer=tf.constant_initializer(0),trainable=is_training)
                cnn_output  = tf.nn.conv1d(inputs, filter_kernels, stride= 1, padding='SAME') + cnn_bias
        return cnn_output
